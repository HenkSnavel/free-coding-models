...
export function buildOpenClawRouterConfig(existingConfig, port, modelId) {
  ...
  // 📖 models: [] is required by OpenClaw's schema — the router handles model selection internally
...
// 📖 configureOpenClawForRouter: Write the OpenClaw config for the router — ONCE at startup only.
// 📖 This function is intentionally called only from the server.listen callback (startup).
// 📖 It is NEVER called from the background ping loop or the request handler.
// 📖 After startup, the router selects the best model dynamically per request without
// 📖 touching ~/.openclaw/openclaw.json again — OpenClaw just talks to http://localhost:{port}
// 📖 and the router internally handles all model switching transparently.
...
export function startRouter(config, cliArgs) {
  ...
  // ─── Background ping loop ───────────────────────────────────────────────────
  // 📖 runPings ONLY updates in-memory candidate state (status, pings).
  // 📖 It NEVER writes ~/.openclaw/openclaw.json — the config is written once at startup
  // 📖 and the router handles dynamic model selection per request without touching the file.
  ...
  server.listen(port, () => {
    ...
    // 📖 ONCE at startup — the config is never rewritten as models go up/down during the session.
    // 📖 After this write, OpenClaw sends all requests to http://localhost:{port} and the
    // 📖 router handles live model selection internally without touching the file again.
    if (cliArgs.openClawMode) {
      try {
        const primaryModel = configureOpenClawForRouter(port, allCandidates)
        const configPath = join(homedir(), '.openclaw', 'openclaw.json')
        console.log(`  🦞 OpenClaw auto-configured! (written once at startup)`)
        console.log(`     Provider: "${OPENCLAW_ROUTER_PROVIDER}" → http://localhost:${port}`)
        console.log(`     Default model: ${primaryModel}`)
        console.log(`     Config: ${configPath}`)
        console.log(`     The router picks the best live model per request — this file won't change again.`)
        console.log()
      } catch (err) {
